...
def trading_detail_before(symbol:str):
  ...
  roc = ta.roc(close=df.Close, length=1)
  cmf = ta.cmf(open=df.Open, high=df.High, low=df.Low,
                      close=df.Close, volume=df.Volume, length=1)  # 柴金资金流量指标
  obv = ta.obv(close=df.Close, volume=df.Volume)  # 能量潮指标   
...
def get_top_30_deal_volume_stocks(pbar=None,dev=False):
    ...
    if dev or is_trading_time():
        ...
        #   df.sort_values(by=['roc','强度'], ascending=[False,False], inplace=True)
