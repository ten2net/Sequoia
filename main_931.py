...
class WeCom:
    ...
    def send_message(self, message):
        ...
        data = {
            "msgtype": "markdown",
            "markdown": {
                "content": f'## <font color="comment">甘州图灵9:31甄选策略{formatted_now}</font> \n\n {message}'
            }
        }
...
def build_markdown_msg(stocks_df):
    ...
    title = " ".join(columns) # ['代码','股票简称','热度排名','开盘', '开盘1分钟价格','开盘1分钟涨幅','最新价','最新涨跌幅']
...
def trading_detail_before(symbol:str):
  ...
  volume_times =( df.iloc[-1]['成交量'] / df.iloc[-6]['成交量'] )> 5
  volume_times_31 = df.iloc[-1]['成交量'] > 3500
...
def get_top_30_deal_volume_stocks(pbar=None,dev=False):
    ...
    if dev or is_trading_time():
        ...
        try:
          ...
          df['开盘1分钟价格'] = df['trading_detail_before'].map(lambda x: x[-5])
          df['开盘1分钟涨幅'] = round(100 * (df['开盘1分钟价格'] - df['开盘']) / df['开盘'],3)
          df['open30'] = df['trading_detail_before'].map(lambda x: x[-9])
          df['volume25'] = df['trading_detail_before'].map(lambda x: x[-8])
          df['price25'] = df['trading_detail_before'].map(lambda x: x[-7])
          df['open31'] = df['trading_detail_before'].map(lambda x: x[-6])
          df['close31'] = df['trading_detail_before'].map(lambda x: x[-5])
          df['high31'] = df['trading_detail_before'].map(lambda x: x[-4])
          df['low31'] = df['trading_detail_before'].map(lambda x: x[-3])
          df['volume31'] = df['trading_detail_before'].map(lambda x: x[-2])
          df['price31'] = df['trading_detail_before'].map(lambda x: x[-1])
          # 进一步刷选出持续强势的票
          df = df.where(df['开盘1分钟涨幅'] < 0.8 * df['最新涨跌幅']).dropna()   
          ...
          df['强度'] = df['开盘1分钟涨幅'] / df['最大可能涨幅'] 
          df.sort_values(by='强度', ascending=False, inplace=True)
          
          directory_path = './results'
          if not os.path.exists(directory_path):
            os.makedirs(directory_path)          
          file_name = f"""甘州图灵甄选931_{datetime.now().strftime('%Y%m%d')}.csv"""
          file_path = os.path.join(directory_path, file_name)

          if not os.path.exists(file_path):
            # 文件不存在，保存DataFrame
            df_save = df[['代码','股票简称','热度排名','开盘', '开盘1分钟价格','开盘1分钟涨幅','最新价','最新涨跌幅',
                            'open30',
                            'volume25',
                            'price25',
                            'open31',
            ...
            # 每天只保存9点31分的热度、出票数据的开盘几分钟交易数据，为了以后进一步优化策略
            pass 
          
          df['upup'] = df['trading_detail_before'].map(lambda x: x[0])
          df = df.where(df['upup'], np.nan).dropna(
              subset=['upup'])          
          selected_stocks = df[['代码','股票简称','热度排名','开盘', '开盘1分钟价格','开盘1分钟涨幅','最新价','最新涨跌幅']]
